...
public class LinkedList<T> implements Iterable<T> {
    ...
    public T kthFromEndRecurs(int k) {
        return kthFromEndRecurs(this.first, k, new AtomicReference<>(0)).item;
    }

    private Node kthFromEndRecurs(Node node, final int k, AtomicReference<Integer> intWrapper) {
        if (node == null) {
            return null;
        }

        Node anode = kthFromEndRecurs(node.next, k, intWrapper);
        intWrapper.set(intWrapper.get() + 1);
        if (intWrapper.get() == k) {
            return node;
        }
        return anode;
